...
- [x] 4. Reasoning-model adapter layer (D39) — **done 2026-07-07 (Phase 4 Block A)**
  ...
  - [x] 4.4 Acceptance verified live 2026-07-07: (a) fake-mode e2e (`AI_FAKE_MODE=reasoning` → FakeReasoningAdapter, usage propagated, $0 cost); (b) live-fire route e2e — admin login → `POST edit-content` with NO model field → `default-reasoning`→gpt-4o, ledger row `feature=admin-edit, provider=openai, modelUsed=gpt-4o, 183/149 tokens, $0.001948` (id cross-checked from `_debug.usage.ledgerId`); (c) **alias switch no-deploy drill**: flipped `default-reasoning` → gpt-4o-mini in DB, same request served by gpt-4o-mini after the 60s registry cache (cost 20× lower), alias restored; (d) suggestTags via `default-cheap` live (gpt-4o-mini). **Provider caveat RESOLVED same day (owner rotated keys):** Anthropic live-fire green — `editContent` via pinned `claude-haiku-4-5-20251001` (202/352 tokens, $0.001962 at the $1/$5 registry rates) + direct adapter tool round-trip with grounded answer; Google live-fire green via `GEMINI_API_KEY` (accepted as fallback for `GOOGLE_API_KEY` everywhere) — `gemini-2.5-flash` chat + functionCall round-trip + grounded answer. Seed pricing rows corrected to the models the account actually serves (`claude-haiku-4-5-20251001` $1/$5, `claude-sonnet-4-5-20250929` $3/$15; stale 2024-dated rows removed — the account 404s on them). **Editing verified live on BOTH configured providers — 4.4 acceptance fully closed.**
  - _Requirements: 4_
