...
| D54 | 🔶 | **Fresh-init migration baseline permitted (owner, 2026-07-03).** All current data is mock and there is **no production database to migrate from**, so Prisma migration history MAY be collapsed to a single `init` migration (schema.prisma is source of truth), hand-augmented with pgvector `CREATE EXTENSION` and the HNSW index DDL (Prisma expresses neither). `db:seed` + `seed:fixture` regenerate all data. No dependencies — do it when convenient; it retires the migration-ordering / `db push`-drift class of bug found in Phase 0. Until then the repaired incremental history (through `20260703193019_semantic_pipeline_catchup`) stands. |
