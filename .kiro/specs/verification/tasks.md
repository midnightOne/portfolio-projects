...
- [ ] 7. Live-fire scripts — *7.1 done 2026-07-12; 7.2 exercised manually 2026-07-05; rest pending*
  - [x] 7.1 `npm run livefire:semantic` (scripts/livefire-semantic.ts) — **done 2026-07-12**: real four-stage ingestion of the fixture through the stage-based pipeline (summaries via the M1 secondary-LLM job path, real embeddings), completion observed via the durable operation row, per-stage outcome + operation-correlated ledger report, hard $0.50 spend cap (run came in at ~$0.001), refuses `AI_FAKE_MODE` without `--allow-fake`. First run green; followed by check:semantic 22/22.
...
- [x] 9. Semantic reliability drills (semantic-content tasks 6.2.2 / 6.3.2 / 10) — **landed + PASSED 2026-07-12.** Two reusable scripted recipes:
  - [x] 9.1 `npm run drill:scope-all` (scripts/drill-scope-all.ts, shared fixtures in scripts/drill-fixtures.ts): deterministic four-project scope:'all' drill, service-level, NO SSE subscriber. `AI_FAKE_MODE=embeddings,reasoning` — since the 2026-07-12 M1 migration the REAL summary path runs keyless through `FakeReasoningAdapter` (the earlier in-script `generateSummary` stub is gone). Injected mid-run scaffold failure on project #2 → parent failed, 1 child failed, 3 completed, marker-vocabulary isolation proven; re-run completes clean. Fresh service instance reads the persisted terminal snapshot (restart/reconnect path). Blast radius: non-drill PUBLIC projects privatized and restored in finally. **The 4 projects are all synthetic** (`drill-delta` carries the nested H2→H3 case) and the REAL kiln fixture is privatized during the run and asserted byte-untouched — an earlier revision fake-ingested the kiln and the damage passed `verify` silently (`check:semantic --no-live` can't tell fake from real; only the live canonical query catches it). If the kiln is ever fake-ingested: `npm run livefire:semantic` restores it.
