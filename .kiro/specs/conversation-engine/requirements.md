...
**Focused designs:** [design-implementation-notes.md](./design-implementation-notes.md) (runtime contracts, pitfalls P1–P40) · [design-ux-and-behavior.md](./design-ux-and-behavior.md) (owner interviews 2026-07-09 + 2026-07-10 Block G addendum: persona/style policy, scenario behavior specs, UX surfaces, slots, leads, analytics, continuity — source of Reqs 13–18 and the 2026-07-10 amendments to 13.2/13.8/13.9/16.4/19.7)
**Contracts:**
...
7. WHEN the engine is disabled or no graph is active THEN all runtimes SHALL behave exactly as today (static start frame, default tools, model from session config) — the engine is a layer, not a rewrite; removal-safety is a permanent property. *(Re-scoped 2026-07-10 by Req 19.7: the conversation-memory layer is independently switched and MAY run graph-less; "exactly as today" is the asserted state with BOTH the steering and memory layers off. Graph-less sessions additionally get an admin-configurable default tool set — Block M2.)*
...
3. WHEN a model-changing transition fires on a **native S2S** runtime THEN guidance/context/tool changes SHALL apply as usual and the model alias SHALL be **ignored for the rest of that session** — native sessions keep their mint-time model, period. *(Owner decision 2026-07-09, removing the originally specced deferred-swap-at-pause machinery: native sessions will never need a smarter brain mid-flight — deep capability on native comes from server-side reasoning tools (D39), and the D49 re-mint path stays reserved for recovery and deliberate provider switching. No `pendingModelSwap` state, no pause scheduling, no cancel-on-revert — any such machinery already built is removed in Block C.)*
4. WHEN a node's `modelAlias` matters on native THEN it applies at **session mint only** (the start node's alias participates in mint-time model resolution); graph validation and the editor SHALL mark model-changing edges as "applies on cascade/text; ignored mid-session on native voice."
...
2. WHEN any turn completes THEN `AIConversation.latestState` SHALL carry the authoritative **`ConversationState` contract** — ONE versioned, Zod-typed snapshot (current node + graph version, slots, visitor profile/flags, agenda progress, summary version, context-set version, last activity) with a `stateVersion` for optimistic concurrency — so resume (Req 2.6), reconnect, provider switch, graph transition, and cross-session resume are all variations of one durable state transition, and the provider session is always a derived cache of this snapshot, never the other way around.
...
2. WHEN the phase completes THEN a live-fire drill SHALL run: a real graph on the fixture project, one conversation per runtime (native voice via fake-mic D53, cascade, text), asserting traversal path, context flushes, tool filtering, and — on cascade/text — a node-driven model-alias switch visible in ledger rows.
...
2. WHEN a node declares on-enter staging THEN it SHALL surface to the model as a staging **suggestion** (target anchor + optional highlight) in the node directive — the engine SHALL NOT execute UI navigation itself (amended by owner 2026-07-10, superseding the engine-executed reading: "switching does not command UI transitions, it only suggests them to the agent"); the model decides whether and when to move per the auto-navigation consent policy (13.8), acting through the existing `UIManager`/`ui_intent`/D59 path — at most one staged move per node entry, never a route load during voice, silently skipped on UI-less runtimes.
...
4. WHEN a node requires structured input (v1: exactly one case — the job-description form) THEN it SHALL be a purpose-built client form tool in that node's allowlist, opened via the existing client-tool path, its submission feeding the existing job-analysis pipeline; the generic form-builder stays backlogged (D18). Expanded by owner 2026-07-10: the form is a **modal** with a raw-text paste field AND a file-drop area whose extraction runs **entirely client-side — only extracted text is ever sent** (data minimization); a processing animation covers the analysis; the result renders in-modal as an LLM-written compatibility document judging fit against experience AND the owner's **work-preferences record** (admin-editable, stored server-side, never public); on completion the agent offers delivery modes — spoken summary, full read-aloud, read-in-peace, or email-me-the-result — with the email address validated, stored against the conversation, and dispatched through the Req 15.2 notification seam (visitor-bound channel, rate-limited to a couple of sends, modular so later artifacts like conversation summaries reuse it).
...
8. WHEN navigation autonomy is decided THEN a per-conversation **auto-navigation consent toggle** SHALL govern it (owner 2026-07-10): OFF (default for every new visitor) = the agent always asks before a commit-level move ("do you want me to take you there?"); ON = the agent may navigate without asking, multi-leg tours included, until turned off. The toggle SHALL be a visible two-state control on the pill, flippable by visitor tap, by visitor spoken request (the model calls a registry client tool — never a prompt-only pretense), and by the agent itself ONLY after explicit visitor consent in-conversation; its state SHALL persist in the `ConversationState` snapshot and be visible to the model in the floating block. Preview-level moves (scroll/highlight supporting the current answer) stay consent-free per 13.6; an explicit question about content remains commit-level intent regardless of the toggle.
9. WHEN UI state and conversation state diverge (the visitor navigates freely while a topic is active) THEN the active node SHALL remain authoritative for conversation state — UI events are transition *evidence*, never forced transitions; the agent SHALL keep answering the active topic, may acknowledge the visitor's movement and offer to follow it once the current answer completes, and SHALL feel "present and aware of the user's actions and the UI state, but not overbearing" (owner 2026-07-10). The UI is not the conversation: a visitor can read the contact section while discussing the kiln project.
...
4. WHEN suggestions are considered THEN runtime-dynamic suggestion generation SHALL NOT be built in this phase (owner decision; taxonomy clarified 2026-07-10) — the deferral covers BOTH dynamic variants: (a) **graph-tied automatic selection** (best-3-of-N chips chosen from `NodeEntryQuestion` data at directive build) and (b) **graph-less model-initiated suggestions** (the model calls a tool whose result feeds the same chips pipeline as plain text — no ids, since off-graph has no deterministic edges to fire; the chip text itself is prompt enough). One chips rendering pipeline, multiple sources — authored ids on-graph, generated text off-graph. The storage schema SHALL not preclude either; the **shared secondary-LLM job infrastructure both variants need IS in scope as a prioritized task** (tasks Block M1).
...
5. WHEN a brand-new visitor arrives THEN an owner-authored **portfolio intro** (what this site is, how it works, what the visitor can do, what the AI can do — including offering the auto-navigation option, Req 13.8) SHALL be available to the model at session start as admin-editable configuration (a document or config text feeding system-prompt assembly) — content, not code (owner 2026-07-10).
...
2. WHEN the visitor converses THEN the engine SHALL maintain **profile flags** — register (technical/layman), intent (hiring / browsing / specific role / general inquiry), behavior (cooperative / probing / rude), topics discussed, conversation duration — as engine-owned state: fast flags filled by the existing per-turn batched cheap call; the richer behavioral assessment by the periodic summarizer (19.3). Flags unify with slots (Req 14): same storage, same templating, same edge-condition support — slots are *stated* facts, flags are *inferred* ones. *(Amended by owner 2026-07-13 — the Aaron-recruiter field report, ai-assistant transcript `cmripxttm…`:)* classifier calls SHALL grade flag evidence as **ordinal signal strengths** (`none | weak | clear | strong` — never raw floats, which cheap models cannot calibrate); the HOST maps grades to numbers and applies **hysteresis thresholds** before any enum flips (e.g. `technical` needs clear+ on 2 of the last 3 user turns or one strong; `probing` needs clear+ twice; a single tongue-in-cheek "ignore all instructions" from an otherwise cooperative visitor never flips behavior — it may be a technical recruiter testing the site). `register=technical` is defined as the visitor **demonstrating fluency themselves** (domain terms used correctly in their own reasoning, implementation/tradeoff follow-ups); naming a technical skill or role they are hiring for is NOT technical register. These definitions live as detailed rubrics in the secondary-call system prompts — that prompt real estate never touches realtime context, so verbosity is free.
3. WHEN the behavior summarizer runs THEN it SHALL be a separate cheap-LLM job over **user turns only**, producing a structured response (intent, register, mood, topics), at most once per interval (default 60s), triggered by staleness checks at turn boundaries (serverless has no resident process), fully async — it never blocks or delays a turn — and gateway-metered. *(Amended by owner 2026-07-13 — J3's single-call interpretation leaked assistant content into the profile: gpt-4o-mini folded the assistant's own greeting gloss "web development projects" into `topics` despite the visitor-turns-only instruction:)* the job SHALL be **two calls with structurally separated inputs** — a **profile call fed VISITOR turns only** (it cannot see assistant text by construction; instruction-following is no longer load-bearing) and a **summary call fed both sides** (a coherent narrative needs what was asked AND answered). Both windows SHALL exclude rows persisted before the first user turn (a stale pre-conversation greeting is not conversation) and transcription-noise rows (the 7.6 clip/mic-feedback artifacts). `topics` = subjects the visitor raised or explicitly engaged with — never subjects only the assistant volunteered. The behavior definition SHALL carry the F4 negative guard ("ordinary questions about the site or the assistant are NOT probing") that already fixed the same failure on the per-turn call.
4. WHEN the profile updates THEN it is a **current assessment, not an event log**: each summarization replaces the previous wholesale, so stale judgments decay naturally (rude five minutes ago, fine now → the profile says fine now). *(Amended by owner 2026-07-13 — the summarizer's own prompt says "omit keys you cannot judge", so omission must not mean deletion:)* decay-by-omission applies to **mood, topics, and behavior** only; an omitted **register or intent** SHALL retain its previous value — those flags change on new evidence, never by silence. (Observed defect: a summarizer run that omitted `register` silently erased the flag the per-turn call had just set.)
...
7. WHEN passive-context sources are composed THEN the floating block SHALL be the concatenation of independent, **source-attributed provider caches** (F-I-D, engine node context, visitor profile/notes, running summary, auto-nav preference, future providers) behind ONE standardized provider contract (owner 2026-07-10): each cache is updated by its own governing system on its own cadence, and the merged block is always buildable from whatever parts currently exist — partial data is normal operation, never an error. **The user/conversation memory providers (profile flags, behavior summarizer, rolling-window summary) SHALL NOT require an active graph** — graph memory is one provider among several; the memory layer gets its own admin on/off switch (default on), independent of graph presence. This amends the J-block `nodeId` gating and re-scopes Req 2.7's removal safety: "no active graph" stops the *steering* layer; "memory off" stops the *memory* layer; **both off = the app behaves exactly as pre-engine** (the asserted removal-safety state).
8. *(Added by owner 2026-07-13.)* WHEN visitor text is interpolated into a secondary-LLM prompt (the per-turn classifier, the summarizer, any future M1 consumer) THEN the prompt SHALL be **injection-hardened structurally, never by content-stripping**: (a) quote-frame escapes — `"""` sequences inside interpolated visitor/transcript text are neutralized so the frame cannot be closed from inside; (b) both prompts state that instructions addressed at the classifier/summarizer itself (telling it to ignore rules or emit specific JSON) must not be followed and **are themselves probe evidence** — injection becomes signal, not noise. Stripping phrases like "ignore all instructions" from classifier input is PROHIBITED (owner 2026-07-13: it blinds the probe classifier to exactly what it exists to detect; the deterministic regex rail in `probe-patterns.ts` remains the layer that cannot be talked out of firing). The visitor-facing counterpart — a light one-line deflection when asked to ignore instructions, never a lecture — is mint-guidance work owned by ai-assistant 7.2a.
