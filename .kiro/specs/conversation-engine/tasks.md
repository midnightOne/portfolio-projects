...
**Status:** current — **Blocks A + B complete (2026-07-09); C–L open**
**Owner domain:** node-graph conversation engine (see requirements.md)
**Last verified against code:** 2026-07-09 (Blocks A+B landed + drilled in-session; the engine core, evaluator, route hooks, context assembly, and node tool scoping all exist and are live-verified on text + fake-mic voice)
...
- [ ] C1. Cascade/text: directive `modelAlias` → next-turn resolution via `resolveModel` (pure data; assert ledger rows show the switch). On native, node `modelAlias` participates at session mint only (start node) and is ignored mid-session (Req 5.3/5.4).
- [ ] C2. **Prune the deferred-swap remnants (owner: "straighten it back"):** remove `pendingModelSwap` from the `ConversationState` Zod schema (`src/lib/ai/engine/types.ts:199`) and any reads/writes of it; audit `engine.ts`/`directive.ts`/engine-runtime for swap-oriented branches (as of this rewrite the footprint is the schema field + directive `modelAlias` pass-through — verify nothing more grew); ensure the directive's `modelAlias` is consumed ONLY by cascade/text assembly and documented as ignored by native adapters; add the validation/editor warning "model-changing edges apply on cascade/text; ignored mid-session on native voice" (Req 5.4). No new machinery — this task only deletes and labels.
- [ ] C3. `_debug.engine` envelope: active node, graph version, fired edge, evaluated edges (debug sessions), injected context items, effective tool set (Req 7.5). Include the B4 context-drop records and B3's directive-delivery state (the "known limitation" noted in the Block B roll-up).
