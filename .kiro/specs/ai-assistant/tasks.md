...
- [ ] 2b. **Restore conversation persistence on the production paths (Req 9.1)** — *owner-priority follow-up to task 2 (2026-07-07); do BEFORE Phase 4. Until this lands, admin replay/analytics/debug browsers only show historical rows and D46 verification step 4 (telemetry reads: conversation log/replay) cannot be satisfied for new conversations.*
  - [ ] 2b.1 **Text path:** `/api/ai/chat` persists each turn server-side via `conversationHistoryManager` — conversation keyed by the gateway session (public `sid` / reflink id), user + assistant `ConversationMessage`s with `transportMode: 'text'`, and `debugInfo` carrying the same systemPrompt/contextString/tool + retrieval traces as the `_debug` envelope so `/admin/ai/conversations` replay and the `/admin/ai/debug` persisted-log browser show real per-turn data. Token counts/cost mirror the ledger row (store the gateway `requestId` for cross-reference). Persistence failures must not fail the chat response (log-and-continue).
  - [ ] 2b.2 **Voice path:** replace the `/api/ai/conversation/log` POST stub with real persistence — transcript items and tool events from the adapters land as messages/metadata on the same `conversationHistoryManager` store (keep the existing debugEventEmitter replay side-channel). The route already receives both batch and per-item formats; persist both.
  - [ ] 2b.3 **D49 forward-compatibility:** do NOT invent leg schema here — conversations stay keyed by sessionId and messages carry `transportMode`; 5b.1 adds legs/snapshots/markers on top of these writes. Coordinate the message shape with 5b.1 so no second migration of message rows is needed.
  - [ ] 2b.4 Acceptance: one public text chat turn and one voice session each produce rows visible in `/admin/ai/conversations` (replay timeline renders) and in the `/admin/ai/debug` recent-sessions browser; `npm run verify`-era telemetry reads (D46 step 4) pass for a fresh conversation.
