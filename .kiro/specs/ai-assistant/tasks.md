...
- [x] 9b. Pre-recorded voice assets (D50) — **done 2026-07-08 (Phase 4 Block C), live-fired**
  - [x] 9b.1 Storage + generation: `VoiceClipPhrase` (admin-managed script — several randomized variants per category) + `VoiceClip` keyed `(voiceId, phraseId)` (migration `20260708033449`); rendered via `lib/ai/tts.ts`, which gained a **google Gemini TTS branch** (raw PCM → WAV wrap) alongside openai/elevenlabs. **Strict voice matching (owner):** one clip set per configured session provider — openai voice via OpenAI TTS, Gemini's prebuilt voice via `default-tts-google` (gemini-3.1-flash-tts-preview), cascade's voice via its own ttsModel; a provider with no matching TTS gets NO clips, never a wrong voice. **Storage deviation (owner-visible, veto-able):** audio bytes live in Postgres, NOT the media pipeline — Cloudinary keys ARE live, but clips are tiny regenerable derived assets, atomic with their key, serverless-honest (D43); `lib/ai/voice-clips.ts` is the single storage seam if the swap is wanted. Routes: public `GET /api/ai/voice-clips/manifest?provider=` + `/audio` (immutable-cached), admin CRUD `/api/admin/ai/voice-clips` + gateway-wrapped `/regenerate` (one ledger row per TTS model; verified: elevenlabs + google + openai rows).
  - [x] 9b.2 Admin page `/admin/ai/voice-clips` (sidebar-linked): phrases grouped by category (custom categories allowed; trigger-wired ones labeled), enable/add/delete, per-voice clip status dots (rendered/stale/missing — stale = text changed since render), per-voice + all-voices regenerate.
