...
|---|---|---|---|---|
| OpenAI Realtime | **primary, implemented** | WebRTC via `@openai/agents` (default) **or `OpenAIRealtimeWebSocket` (task 7.3, 2026-07-15)** — WS auto-selected on Firefox (the WebRTC↔OpenAI ICE-consent interop bug, openai-agents-js#1353, kills Firefox sessions ~36s in) or forced via voice-config `sessionConfig.transport:'websocket'`. The WS transport carries events only: the adapter owns mic capture (PCM16 24k → `sendAudio`; mute + the 7.6 clip gate are frame drops — `session.mute` throws on WS) and playback (scheduled buffers → MediaStreamDestination → the same audio element, so output mute/volume/meter are transport-agnostic); disruption watch polls socket status (no blip/grace — a closed WS never self-recovers) | `POST /api/ai/openai/session` | Ephemeral session; prompt + tools at mint; model via `default-realtime` alias (D4); on WS the minted model rides `session.connect({model})` (the WS URL needs it) |
...
**Implementation shape:** `VoiceClipPhrase`/`VoiceClip` tables — audio in the MEDIA PIPELINE (owner, 2026-07-08: Cloudinary serves audio via its `video` resource type; rows hold the versioned CDN url + public id; regeneration overwrites the public id in place; same upload path future full-conversation debug recordings will use), strict per-provider voice matching (openai voice / Gemini prebuilt voice via Gemini TTS / cascade voice — no clips beats a wrong voice), `ClipPlayer` with priority round-robin caching + randomized pick among loaded variants, triggers in the conversational-agent provider (filler at +1.2s off `tool_call_start`, cutoff on the new `speech_start` audio event; D49 connection clips; optional greeting), `clip_played` history rows rendered as a distinct replay step, and `firstAudioAt` turn-onset capture (exact Gemini, best-effort OpenAI). Admin page: `/admin/ai/voice-clips` with per-voice preview playback.

**Latency-aware narration (owner, 2026-07-08 — `lib/ai/tool-latency.ts`):** per-tool median execution times (measured from persisted tool rows, which cover client tools like ui_intent that never touch the ledger) are injected into BOTH native mints as a TOOL LATENCY AWARENESS block — instant tools (<600ms): never narrate, act silently and describe the result; fast (<1.5s): a few words at most; slower: a short CONTEXT-RELEVANT lead-in (never canned phrases — "filler is context-dependent for humans"). The clip player covers true overruns independently, so the model never needs to cover silence itself. Verified live: a Gemini nav request produced zero action-narration, and the clip fired only because that turn's tool chain genuinely ran long. Dynamic contextual filler (topic-aware generated lead-ins) is the named later step.

**Prompt/key security (owner audit, 2026-07-08):** the public `/api/ai/voice-config` read now strips `instructions` and `tools` alongside env-var names — the persona prompt was publicly readable. Both native providers get the full prompt injected ONLY server-side at mint (Gemini: locked into the ephemeral token; OpenAI: client_secrets session config), and the OpenAI adapter's dead client-side prompt fallback was deleted (an agent-side value would override the minted instructions via session.update). API keys never reach the client on any path.
