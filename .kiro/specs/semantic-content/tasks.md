...
**2026-07-12 (post-M1 hygiene pass, D42 spirit):** legacy one-shot ingestion deleted — `/api/admin/semantic/ingest` route + `ContentIngestionService` (zero UI consumers; superseded by `processing/start`; carried 3 direct OpenAI calls), `SmartContentGenerator.generateHierarchicalContent` and its entire dead helper chain incl. the never-called raw-OpenAI `generateAISummary`/`generateSectionSummary` (file 1362→~370 lines; scaffold path is now the ONLY generator and holds no provider client), the unused OpenAI client in `StageBasedProcessingService`. `SemanticDiagnosticService`'s T3 test rewired to `generateScaffoldOnly` — same `T3HeadingBoundedChunking` output, now AI-free/keyless (`diagnostics:t3` verified healthy 3/3). Direct provider SDK usage in `src/lib/content/` is now exactly one file: `BatchEmbeddingService` (OpenAI Batch API, spec-frozen per Req 6.3).

**2026-07-12 (M1 alignment, same session as Req 9 closure):** T1/T2 summary generation migrated off the raw OpenAI SDK onto `runSecondaryLLMJob` (conversation-engine M-block — the required path for all secondary-LLM features): alias-resolved (`default-cheap` default, admin custom model via the module's new `modelOverride`), AI_FAKE_MODE=reasoning-aware (drill:scope-all dropped its in-script summary stub and runs keyless), JSON-forced `{"summary"}` with a raw-text salvage for non-compliant/truncated output, budget deduction + D32 ledger mirror ride the module's custom meter with the ACTUAL provider/model recorded (was hardcoded `provider:'openai'`). `BudgetAwareAIOperations` no longer holds a provider client or requires `OPENAI_API_KEY`. Verified: drill:scope-all green stub-free, new `npm run livefire:semantic` real ingest $0.0009, check:semantic 22/22, jest 1050 green.
