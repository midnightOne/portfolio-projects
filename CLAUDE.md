...
5. **Deployment state (owner, 2026-07-03; creds updated 2026-07-09):** the project is currently **out of deployment**; iterate locally. Fresh, working credentials: `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `GEMINI_API_KEY` (in `.env.local`), and Cloudinary. Cloud DB (Supabase/Neon) and Vercel registration get re-provisioned only when deploying again.
...
npm run check:scenarios # golden scenarios through the REAL engine against fakes (P16, zero spend); --rebaseline repins diverged expectations, --graph <id> scopes
npm run verify         # umbrella: type-check + check:gateway + check:specs + check:semantic --no-live + check:scenarios
```

**Fixture needs ingestion to get embeddings.** `npm run seed:fixture` creates the fixture *project* but not its chunk embeddings — those come from the ingestion pipeline. After a fresh `db:reset`/`seed:fixture`, ingest the fixture (POST `/api/admin/semantic/processing/start` with `scope:'project'`, `projectId`, all four stages `immediate`) before `check:semantic`'s live query will pass. The same applies to the three seeded sample projects (`portfolio-website`, `llm-systems-research`, `e-commerce-platform` — deep articles since 2026-07-17, content lives in `prisma/seed-articles.ts` and refreshes on reseed): ingest each after a reset, or use `scope:'all'`. Trap: `projectId` here is the project's DB **id (cuid), not the slug** — a slug fails with "No project found for request". Stage names are `chunking`/`summaries`/`embeddings`/`validation`. `seed:fixture` also seeds the `doc:fixture-resume` document source (ai-assistant 7.15) — ingest it with `scope:'entity'`, `sourceId:'doc:fixture-resume'` for cross-source retrieval drills; document sources are managed at `/admin/ai/content-sources` (the config is the ingestion manifest + a query-time allowlist). `scope:'all'` is supported since 2026-07-12 (reliability gate passed): it runs one durable child operation per PUBLIC project with content; parent + children live in `semantic_processing_operations` (queue/SSE are projections of that table — status is durable with no SSE subscriber). Stage objects need `enabled: true` alongside `stage`/`mode`. Reliability drills: `npm run drill:scope-all` (service-level, fake AI) and `npm run drill:semantic-reliability` (HTTP, real AI, needs the dev server + admin creds; use a FRESHLY started server after editing pipeline code — global singletons survive HMR).

Conversation-engine extras: `npx tsx scripts/seed-conversation-graph.ts [--publish]` seeds the §8 catalog graph + its golden-scenario behavior suite (F3; idempotent, stable node ids; publish embeds exemplars — pennies). Engine batch jobs (question analytics, summary backfill + retention sweep) are admin/cron POSTs: `/api/admin/ai/engine/batch/{questions,summaries}` — also buttons on the conversation-graphs admin page; cron wiring is a deploy-time step.

Planned (verification spec, land with their phases): `check:models`, `livefire:chat`.

## Environment

`.env` needs: `DATABASE_URL` (Postgres with pgvector), `NEXTAUTH_SECRET`/`NEXTAUTH_URL`, `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `GOOGLE_API_KEY` (Gemini Live voice + Gemini TTS; `GEMINI_API_KEY` — Google's documented name — is accepted as a fallback everywhere), `ELEVENLABS_API_KEY` (cascade TTS/STT engine). Media pipeline (project media AND the D50 voice clips; future conversation recordings): `MEDIA_PROVIDER=cloudinary` + `CLOUDINARY_CLOUD_NAME`/`CLOUDINARY_API_KEY`/`CLOUDINARY_API_SECRET`/`CLOUDINARY_FOLDER`. Email (Req 15.2 notification seam, provider **Resend**): `RESEND_API_KEY` (unset = well-defined "unconfigured" — captures durable, sends logged as skipped, dispatchable from `/admin/ai/job-analysis`), `EMAIL_FROM` (deploy-time: verified domain; default is Resend's dev-only sender), `OWNER_NOTIFY_EMAIL` (owner pushes + Reply-To on visitor mail), optional `VISITOR_EMAIL_MAX_PER_CONVERSATION` (default 2). API keys live in env only — never DB (D3). Dev-only flags: `DEV_VERIFICATION=true` (debug envelope), `AI_FAKE_MODE=reasoning,voice,embeddings,email` (test doubles) — both refuse production. Optional `AI_SESSION_SECRET` for public chat JWTs (falls back to `NEXTAUTH_SECRET`).
