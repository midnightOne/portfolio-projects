...
- `.kiro/specs/ai-assistant/tasks.md` **7.1a + 7.1e + 7.1f** (fid diet + `ui_details` pull tool), **7.2c as finalized** (owner bio + what-the-portfolio-is move into the mint instructions via `assembleStartFrame()`; the fid `frame` object dies), **7.11** (resume speech policy), **7.12** (reflink entry field), **7.13** (`portfolio_overview` tool). If you touch mint guidance, the **7.2a** rider (soft probe-deflection line) rides along; otherwise leave 7.2a for its own session.
- `.kiro/specs/mcp-server/tasks.md` **task 6** (`portfolio_overview` via MCP — shared implementation with 7.13).
...
5. **7.2c (finalized) + 7.1a + 7.1e + 7.1f** identity-to-mint + fid diet + pull tool — do these together, they are one coherent change: extend `src/lib/ai/start-frame.ts` so `assembleStartFrame()` carries the owner bio + the what-this-portfolio-is/how-it-works framing, and wire it into the OpenAI mint (GET **and** POST — Google already calls it); then the compact-text fid publish in `src/lib/ai/PassiveFIDManager.ts` → `UIManager` seam drops the `frame` object entirely (bio/capabilities now live at mint), keeps `briefSummary` + `detailedSummary`, drops `projectSummary`; new client tool `ui_details` in the unified registry (`src/lib/ai/tools/`) reading the retained client-side buffer — zero server round-trip; no-repeat-if-NAV_CONTEXT-unchanged rule in its description. Mind the OpenAI adapter's `_compactFidForTransport` (it compacts the fid JSON pre-publish — the diet may make it dead weight; verify, don't assume). Keep the mint-route `mark()` checkpoints honest when adding the start-frame section (the 7.0 panel's mint tab depends on them — add a `mark('start frame')` like the Google route has).
5b. **7.13 + mcp-server task 6** `portfolio_overview`: server tool in the unified registry, `depth: 'brief' | 'full'` only (schema stays tiny — it is minted into every session); `brief` = the `assembleStartFrame()` artifact verbatim, `full` = owner bio T1 + visitor-intro chunk + project index; expose the same implementation via the MCP server (update `McpConnectCard` on /about/ai + README tool count 3 → 4). One assembly module for mint, tool, and MCP — never fork it.
...
`npm run verify` green (type-check + check:gateway + check:specs + check:semantic --no-live + check:scenarios — expect scenario updates if flag semantics change golden behavior suites; use `--rebaseline` only with justification recorded in the ledger). Then drive it live: a fake-mic or text-only session reproducing the field reports — (a) a recruiter saying "firmware engineering" must NOT flip register technical on one turn; (b) the summarizer must not surface assistant-volunteered topics; (c) `ui_details` answers an on-screen question with no server round-trip and is not re-called in an unchanged view; (d) a second forced resume reconnects silently; (e) with the frame moved to mint, a fresh session still answers "who is Kirill?" grounded (mint-carried identity) and "tell me more about Kirill" in depth via `portfolio_overview` — plus one raw MCP `tools/call` for the overview (the mcp-server 5.1 curl pattern). Update the Block N / 7.x / mcp-server 6 checkboxes with evidence in the house style, bump spec status headers, run `npm run check:specs`, and commit to staging with `git commit -F -` (PS 5.1 quoting trap).
