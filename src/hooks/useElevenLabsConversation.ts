 ...
 * 
 * React hook for managing ElevenLabs Conversational AI sessions with signed URL conversations.
 * Provides conversation lifecycle management, connection state, and error handling.
 */
...
interface UseElevenLabsConversationReturn {
  ...
  // Adapter access
  adapter: ElevenLabsAdapter | null;
}
...
export function useElevenLabsConversation(
  ...
  // Refs
  const adapterRef = useRef<ElevenLabsAdapter | null>(null);
    ...
    try {
      ...
      const initOptions: AdapterInitOptions = {
        ...
        onConnectionEvent: (event) => {
          ...
          if (event.error) {
            const error = new VoiceAgentError(event.error, 'elevenlabs');
            setLastError(error);
            onError?.(error);
          } else {
            setLastError(null);
  ...
  }, [currentAgentId, voiceId, onConnectionChange, onSessionChange, onTranscriptUpdate, onError]);
  ...
  // Update agent ID when it changes
  useEffect(() => {
    if (agentId && agentId !== currentAgentId) {
      setCurrentAgentId(agentId);
      
      // Reinitialize adapter with new agent ID
      if (isInitializedRef.current) {
        isInitializedRef.current = false;
        initializeAdapter();
      }
    }
  }, [agentId, currentAgentId, initializeAdapter]);
